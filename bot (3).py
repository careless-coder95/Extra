#!/usr/bin/env python3
"""
OSINT Telegram Bot - Complete Standalone Version
Edit the configuration section below and run: python bot.py
"""
...
try:
    ...
    print("pip install python-telegram-bot pymongo aiohttp python-dotenv")
    exit(1)
...
try:
    ...
    print(f"❌ Cannot connect to MongoDB. Check your MONGO_URI!")
    exit(1)
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    if "terminated by other getUpdates request" in str(context.error):
        logger.warning("⚠️ Conflict detected: Another instance is running")
...
def main():
    ...
    if not BOT_TOKEN or BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        print("❌ BOT_TOKEN not configured! Edit the file and add your bot token.")
        return
    
    if not OWNER_ID or OWNER_ID == "YOUR_OWNER_ID_HERE":
        print("⚠️ OWNER_ID not set! Owner commands will not work.")
    
    print("=" * 60)
    print("🤖 OSINT Telegram Bot - Starting...")
    print("=" * 60)
